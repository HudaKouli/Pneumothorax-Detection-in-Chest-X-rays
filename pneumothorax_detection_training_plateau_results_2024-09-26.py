...
file_name = 'pneumothorax_detection_training_plateau_results_2024-09-26.xlsx'
...
def plot_results(file_name):
    ...
    # Loss Plot
    plt.subplot(1, 2, 1)
    plt.plot(df['Epoch'], df['Train Loss'], label='Train Loss', marker='*', linestyle = ':',color=plt.cm.viridis(0.1),linewidth = 2)
    plt.plot(df['Epoch'], df['Val Loss'], label='Validation Loss', marker='*', linestyle = ':',color=plt.cm.viridis(0.4),linewidth = 2)
    ...
    plt.legend(loc='upper left',bbox_to_anchor=(1, 1), 
        framealpha=0.5, 
        facecolor='black', 
        edgecolor='white',prop={'size': 12,'family':'Times New Roman' })
    plt.xlim([0, max(epochs_range)]) 
    plt.grid(True,which='both', color='lightgray', linestyle='--')

    # Accuracy Plot
    plt.subplot(1, 2, 2)
    plt.plot(df['Epoch'], df['Train Accuracy'], label='Train Accuracy', marker='*',linestyle = ':',color=plt.cm.viridis(0.2),linewidth = 2)
    plt.plot(df['Epoch'], df['Val Accuracy'], label='Val Accuracy', marker='*',linestyle = ':',color=plt.cm.viridis(0.5),linewidth = 2)
    plt.xlabel('Epoch', lables_properties)
    ...
    plt.savefig("training_with_plateau_plot.svg", format="svg")
    plt.show()
